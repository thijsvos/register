...
| 3 | **`zoom` × `dvh` is proven in one engine**<br>The e2e suite is Chromium only. Zoom-on-root interacting with `dvh` is exactly where engines differ, and the assertion that would catch a clipped status bar runs nowhere else. | Adding Firefox and WebKit projects is config, not doctrine. |
| 4 | **`git status` runs on every tree fetch**<br>The tree is fetched after every event burst. Cheap here, repeated forever, and a short-lived cache is the obvious answer. | Pure performance, no contract in the way. |
| 5 | **No WebSocket keepalive**<br>No ping/pong and no send timeout, so a half-open connection is never noticed. Harmless on loopback, real over a tailnet — which shipped in P12. | The trigger named P12 remote mode, and that is here. |
