...
| Item | Why it is parked | Trigger |
|---|---|---|
| **`docker compose up` is unverified** | P10's first acceptance clause. The UI stage builds — pnpm resolves from `packageManager` exactly as intended — but the machine's Docker VM is at 71 GB inside a 96 GB virtual disk and returns `input/output error` writing its own buildkit metadata, so no build can finish here. The server stage was proven a different way: a compile from *only* the five paths the Dockerfile copies, which is the failure this would actually have caught. `tests/release.rs` pins the rest of the file's shape. | A working Docker daemon. One `docker compose -f deploy/docker-compose.yml up` settles it. |
